...
public class Player extends Element {
    ...
    public void moveByBall(int ball_x, int ball_y, float speed_x, float speed_y, boolean ifBotMistake) {
        ...
        if (ifStart == true) {
            ...
            if (speed_y > 0 && ball_x > (Content.STICK_X - 40)) {
                ...
                // ball will touch the right border -> rebound
                if (predict_x > Content.FRAME_WIDTH)
                    predict_x = Content.FRAME_WIDTH - (predict_x - Content.FRAME_WIDTH) - 20;

                // when ball is fast enough, allow the bot to make mistake
                // if (ifBotMistake && (Math.abs(speed_x) > 6) && (Math.abs(speed_y) > 11))
                // predict_x += (70 + Math.random() * 90);
            ...
            if (hit_x - predict_x > 10) {
                ifLeft = true;
                ifRight = false;
                // let the bot react slowly after catch the ball -> make it stupid
                if (ifBotMistake && speed_y < 0 && ball_y - this.y < 10)
                    ifLeft = false;
            ...
            /*
             * bot's jump condition: only when the player's score > 5 , the ball will drop
             * in front of the bot, and they r close enough; if bot meets the jump
             * condition, decide whether to jump by jumpRand;
             */
            double jumpRand = Math.random();
            if (!ifBotMistake && -60 < (ball_x - this.x) && (ball_x - this.x) <= 0 && speed_y > 0
                    && Math.abs(this.y - ball_y) < 70 && ifJump == false && jumpRand > 0.3) {
                ifJump = true;
